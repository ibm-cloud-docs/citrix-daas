...
|Volume Worker | n/a | n/a | \n - 1 IBM Cloud Databases for Redis  with 2G RAM 4G of storage  \n - IBM Cloud Functions instance   \n - 1 Cloud Object Storage bucket|

Lite is for testing and proof of concept, not production. 

[^lite]:Lite is 2 vCPU. 
[^masterimage]:Master image vsi is temporary and is used to create the machine catalog.  The master image VSI is automatically created and reclaimed by {{site.data.keyword.cvad_short}}. 
...
## Step 3 - Infrastructure location and sizing configuration
...
* Size of your control plane infrastructure, either Lite, Standard, or Enterprise with Volume Worker. Lite is designed for testing and proof of concept. Enterprise includes Volume Worker with is used for distributed identity disk creation with large machine catalogs.

## Step 4 - Dedicated Host Specifics
...
## Step 5 - Images and profiles for Machine Catalog
...
1.  Indicate whether you want to use a Custom image to build the base image for your machine catalog.
    *  Select the operating system and version for the custom image.
...
*   Cloud Connector VSIs (Connector Logs and Plugin), 
*   Domain Controller logs from the Active Directory Server, 
*   Volume Worker logs that are on both IBM Cloud Functions and the worker VSI logs that complete the jobs.
...
*   Create a new instance and select a logging plan to use. Set whether to enable platform. Platform logging enables logging for the volume worker manager on a {{site.data.keyword.la_full_notm}} instance. Only one instance of {{site.data.keyword.la_full_notm}} per region can be enabled for platform logs. 
